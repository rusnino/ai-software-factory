...
[^phase1-verification]: `VerificationService` is invoked by `EventBridge.handle()` on `landing:completed`, executes `required`/`optional` `Check` commands via real subprocess with real exit-code comparison, and gates `AGENT_REVIEW -> HUMAN_REVIEW` vs `FAILED` on the result. Both the failing and passing paths are covered by automated tests (GAP-006, GAP-021 closed). REVIEW-013 found `verify_and_advance()`'s own concurrent-modification branch raised without committing first, the same defect class as GAP-044 in the sibling file — `GAP-058` (HIGH), fixed by `d7ed2a4` and confirmed by live reproduction in REVIEW-014, `CLOSED`. REVIEW-016 found a separate, still-open issue in this same area: `TaskContract.forbidden_paths` is silently dropped by `verify_execution()` whenever a `CompletionContract` is attached (only checked in the no-`CompletionContract` branch) — tracked as `GAP-074` (HIGH, OPEN).
[^phase1-durability]: `get_db()` commits per-request sessions on success and rolls back on exception (GAP-022 closed). Audit rows for rejected approvals are committed before the rejection response is returned (GAP-044 closed). REVIEW-013's `GAP-058` finding (one remaining write-then-raise site without a preceding commit, in `VerificationService.verify_and_advance()`'s CAS-failure branch) is fixed and `CLOSED` (`d7ed2a4`, confirmed REVIEW-014/016). **Unchecked in REVIEW-016**: all of the above was verified only against SQLite. Every `datetime` column in every model is `TIMESTAMP WITHOUT TIME ZONE` while every `utc_now()` call produces a timezone-aware value; `asyncpg` rejects the mismatch outright. Live-reproduced against a real Postgres container: `POST /tasks` fails on its very first write. Task storage is not currently durable — or functional at all — against the database this project mandates as its Phase 1 target. Tracked as `GAP-078` (CRITICAL, OPEN) — this is now the gap that blocks declaring Phase 1 acceptance criteria met.
[^phase1-policy-bypass]: REVIEW-013 found that `PolicyEngine`'s and `VerificationService`'s forbidden-path checks (`policy_engine.py::_is_inside`, `verification_service.py::_is_prefixed_by`) compared raw strings after only `rstrip("/")`, with no `..`-traversal normalization. `GAP-057` (CRITICAL), fixed by `aaca2cc`/`utils/paths.py::normalize_path()` and confirmed by live reproduction in REVIEW-014, `CLOSED`. REVIEW-016 found a related but distinct issue still open: `PolicyEngine.evaluate()` never reads `TaskContract.forbidden_paths` at all (only `ProjectProfile.security.forbidden_paths`) — `GAP-074` (HIGH, OPEN) — see `[^phase1-verification]`.
