...
[^phase1-self-approval]: This checkbox covers that an approval step durably exists, not that its actor identity is trustworthy end-to-end. Round 30 (2026-09-14) found and live-reproduced `#376` (HIGH): the self-approval guard in `approval_service.py` compares the approving actor against `TaskContract.proposed_by`, a free-form client-supplied field with no authenticated binding to the caller — so one party can set an arbitrary `proposed_by` and then approve its own task through both PLAN and EXECUTION with zero rejection, violating CLAUDE.md's "Do not allow agents to approve their own work" rule. A round-30 fix attempt (`ad1ebf8`, an opt-in `GC_KNOWN_PROPOSERS` allow-list) briefly closed this but was reopened in round 31 (2026-09-15) after live re-verification found it's a no-op in every default deployment. A 3rd fix attempt (`65af15f`, round 33, 2026-09-16) makes `GC_KNOWN_PROPOSERS`/`GC_ADMINS` fail closed by default and adds a genuinely independent second credential — `GC_HUMAN_APPROVAL_SECRET`, required via a new `X-Human-Approval-Secret` header for EXECUTION/MERGE approval, never settings-derived, distinct from the general `GC_CONTROLLER_API_SECRET` — closing the round-31 "propose as one known name, approve as another" residual. **CONFIRMED `CLOSED` as of round 33**: independently reproduced both round-31 bypass scenarios through the real HTTP API pre-fix (both succeeded) and post-fix (both correctly rejected with 403), the first of 3 fix attempts to hold up under live end-to-end re-verification. One functional regression was introduced by the fix itself: `#390` (HIGH) — the CLI's `gc approve` was never updated to send the new header, so its default EXECUTION approval type now always 403s in every deployment.
...
recurrence of the same test-masking defect) and `#366`'s branch-protection claim didn't match the
live API. Fresh-angle pass found `#376` (self-approval) and `#377` (unbounded-nesting 500).
