...
| RISK-21 | A successful macro-agent `POST /runs` can be accepted externally while its response is lost or malformed, leaving a Controller execution without the external run ID and allowing recovery to start duplicate/orphaned work | Require macro-agent idempotency keyed by `controller_execution_id` or a lookup API before treating response-loss recovery as complete; currently blocked by the Phase 2 constraint against modifying macro-agent internals; tracked in GitHub issue #301 | SPEC-05, ADR-002 |
